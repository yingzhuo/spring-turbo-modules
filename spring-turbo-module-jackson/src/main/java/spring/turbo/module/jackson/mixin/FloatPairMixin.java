...
public abstract class FloatPairMixin {
    ...
    static class FloatPairJsonDeserializer extends JsonDeserializer<FloatPair> {

        @Override
        @Nullable
        public FloatPair deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
            final String source = p.readValueAs(String.class);
            return (FloatPair) CONVERTER.convert
                    (
                            source,
                            TypeDescriptor.valueOf(String.class),
                            TypeDescriptor.valueOf(FloatPair.class)
                    );
        }
    }
